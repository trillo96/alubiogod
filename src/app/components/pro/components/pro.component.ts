...
export class ProComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subject
      ...
      .subscribe(() => {
        ...
        this.usersNombre=this.userService.getUsuariosPorNombre(this.users,this.nombreFiltro);
        this.usersTemplate=this.usersNombre.slice(0,20);
      }   
      else{        
        this.usersTemplate=this.users.slice(0,20);
      }
  ...
  add20users() {   
    if(this.formGroup.valid){
      this.usersTemplate=this.usersTemplate.concat(this.usersNombre.slice(20*this.page, 20*(this.page+1)));
  ...
  onScroll() {
    if(this.formGroup.valid){
      if (this.usersTemplate.length < this.usersNombre.length) {
        setTimeout(()=>{
          this.add20users();
          this.page ++;
